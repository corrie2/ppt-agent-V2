...
def build_pptx(
    ...
    for slide_spec in spec.slides:
        ...
        _add_source_footer(slide, slide_spec, evidence_by_id=evidence_by_id)
...
def _render_academic_cover(slide, slide_spec: SlideSpec) -> None:
    band = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, 0, Inches(4.45), SLIDE_HEIGHT)
    band.fill.solid()
    band.fill.fore_color.rgb = NAVY
    band.line.color.rgb = NAVY
    _add_textbox(slide, Inches(0.65), Inches(0.7), Inches(3.3), Inches(0.35), "PAPER BRIEF", 11, True, TEAL)
    _add_textbox(slide, Inches(0.65), Inches(1.35), Inches(3.35), Inches(3.0), slide_spec.title, 25, True, WHITE)
    _add_textbox(slide, Inches(0.65), Inches(4.65), Inches(3.25), Inches(1.1), _join_lines(_visible_bullets(slide_spec)), 13, False, WHITE)
    _add_textbox(slide, Inches(5.0), Inches(1.2), Inches(6.9), Inches(1.35), _slide_message(slide_spec), 24, True, NAVY)
    _draw_evidence_motif(slide, Inches(5.0), Inches(3.0), Inches(6.9), Inches(2.6))
...
def _render_title_cover(slide, slide_spec: SlideSpec) -> None:
    ...
    _add_textbox(slide, Inches(0.8), Inches(1.7), Inches(5.8), Inches(1.2), _slide_message(slide_spec), 18, False, WHITE)
    _add_textbox(slide, Inches(0.8), Inches(3.0), Inches(4.8), Inches(2.2), _join_lines(_visible_bullets(slide_spec)), 15, False, WHITE)
    _render_visual_area(slide, slide_spec, Inches(7.0), Inches(0.7), Inches(5.1), Inches(5.5), accent=TEAL)


def _render_title_bullets(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.85), Inches(1.15), Inches(11.4), Inches(0.7), _slide_message(slide_spec), 18, True, BLUE)
    ...
    for index, bullet in enumerate(bullets[:3]):
        ...
        card.line.color.rgb = TEAL if index == 1 else MID
        badge = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.OVAL, left + Inches(0.22), Inches(2.35), Inches(0.45), Inches(0.45))
        badge.fill.solid()
        badge.fill.fore_color.rgb = BLUE if index != 1 else TEAL
        badge.line.color.rgb = badge.fill.fore_color.rgb
        _set_text(badge.text_frame, str(index + 1), 10, True, WHITE, center=True)
        _add_textbox(slide, left + Inches(0.25), Inches(3.0), Inches(2.95), Inches(1.7), bullet, 15, False, DARK)


def _render_hero_image_plus_argument(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.5), Inches(4.7), Inches(0.8), _slide_message(slide_spec), 22, True, NAVY)
    _add_textbox(slide, Inches(0.8), Inches(2.4), Inches(4.7), Inches(2.8), _join_lines(_visible_bullets(slide_spec)), 16, False, DARK)
...
def _render_two_column_text_image(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.4), Inches(5.1), Inches(0.7), _slide_message(slide_spec), 17, True, BLUE)
    _add_textbox(slide, Inches(0.8), Inches(2.1), Inches(5.0), Inches(2.8), _join_lines(_visible_bullets(slide_spec)), 15, False, DARK)
...
def _render_three_card_summary(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.3), Inches(11.4), Inches(0.6), _slide_message(slide_spec), 18, False, DARK)
    ...
    for idx, item in enumerate(items):
        ...
        _set_text(shape.text_frame, item, 16, False, NAVY)


def _render_process_timeline(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.3), Inches(11.2), Inches(0.7), _slide_message(slide_spec), 17, False, DARK)
...
def _render_figure_with_caption(
    ...
    *,
    ...
    if has_bullets:
        ...
        _add_textbox(slide, Inches(8.65), Inches(2.3), Inches(3.65), Inches(3.0), _join_lines(bullets), 13, False, DARK)
...
def _render_multi_figure_slide(
    ...
    *,
    ...
    if len(figure_ids) <= 1:
        _render_figure_with_caption(slide, slide_spec, figures_by_id=figures_by_id, evidence_path=evidence_path)
        return

    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.85), Inches(1.15), Inches(11.5), Inches(0.55), _slide_message(slide_spec), 14, True, NAVY)
    panels = [
    ...
    for figure_id, (left, top, width, height) in zip(figure_ids[:2], panels):
        ...
        frame.line.color.rgb = MID
        if image_path and image_path.exists():
            _add_picture_contain(slide, image_path, left + Inches(0.1), top + Inches(0.1), width - Inches(0.2), height - Inches(0.55))
        else:
            _draw_missing_figure_placeholder(slide, slide_spec, left + Inches(0.1), top + Inches(0.1), width - Inches(0.2), height - Inches(0.55), figure_id=figure_id)
        caption = _figure_caption(figure, slide_spec, figure_id=figure_id)
        _add_textbox(slide, left + Inches(0.18), top + height - Inches(0.42), width - Inches(0.36), Inches(0.32), caption, 8, False, GRAY)
    bullets = _visible_bullets(slide_spec)
    if bullets:
        _add_textbox(slide, Inches(0.95), Inches(5.9), Inches(11.1), Inches(0.75), _join_lines(bullets), 12, False, DARK)
...
def _render_method_figure_callouts(
    ...
    *,
    ...
    _add_textbox(slide, Inches(8.45), Inches(1.25), Inches(3.9), Inches(0.7), _slide_message(slide_spec), 15, True, NAVY)
    for index, callout in enumerate(callouts[:3]):
        label = getattr(callout, "label", None) or callout.get("label", str(index + 1))
        text = getattr(callout, "text", None) or callout.get("text", "")
        ...
        _set_text(badge.text_frame, str(label), 10, True, WHITE, center=True)
        _add_textbox(slide, Inches(9.05), top - Inches(0.05), Inches(3.1), Inches(0.8), text, 12, False, DARK)
    caption = _figure_caption(figure, slide_spec, figure_id=figure_id)
    reason = slide_spec.content.visual_reason or slide_spec.image_rationale
    footer = f"{caption}\n{reason}" if reason else caption
    _add_textbox(slide, image_left, Inches(6.15), Inches(11.4), Inches(0.6), footer, 10, False, GRAY)


def _render_result_table_summary(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.25), Inches(11.4), Inches(0.7), _slide_message(slide_spec), 18, True, NAVY)
    summaries = [
    ...
    for index, summary in enumerate(summaries[:3]):
        ...
        _set_text(card.text_frame, summary, 15, True if index == 0 else False, NAVY if index == 0 else DARK)
    ...
    _add_textbox(slide, Inches(0.85), Inches(5.25), Inches(11.3), Inches(0.45), trace, 11, False, GRAY)
    _add_textbox(slide, Inches(0.85), Inches(5.75), Inches(11.3), Inches(0.45), citations, 10, False, GRAY)


def _render_ablation_summary(slide, slide_spec: SlideSpec) -> None:
    _add_section_title(slide, slide_spec.title)
    _add_textbox(slide, Inches(0.8), Inches(1.25), Inches(11.4), Inches(0.65), _slide_message(slide_spec), 17, True, NAVY)
    points = _visible_bullets(slide_spec)
    ...
    for index, (heading, text) in enumerate(columns):
        ...
        panel.line.color.rgb = TEAL if index == 1 else MID
        _set_text(panel.text_frame, f"{heading}\n{text}", 14, True, DARK)
    _add_textbox(slide, Inches(0.85), Inches(5.8), Inches(11.2), Inches(0.5), ", ".join([c.evidence_id for c in slide_spec.citations[:4]]), 10, False, GRAY)


def _render_big_quote(slide, slide_spec: SlideSpec) -> None:
    _add_textbox(slide, Inches(0.9), Inches(0.65), Inches(10.8), Inches(0.45), "KEY TAKEAWAY", 11, True, TEAL)
    _add_textbox(slide, Inches(0.9), Inches(1.45), Inches(10.9), Inches(2.0), _slide_message(slide_spec), 26, True, NAVY)
    bullets = _visible_bullets(slide_spec)
    ...
    for index, bullet in enumerate((bullets + ["", "", ""])[:3]):
        ...
        panel.fill.solid()
        panel.fill.fore_color.rgb = WHITE
        panel.line.color.rgb = TEAL if index == 0 else MID
        _set_text(panel.text_frame, bullet, 13, False, DARK)
...
def _draw_two_column_fallback(slide, slide_spec: SlideSpec, left, top, width, height, *, accent: RGBColor) -> None:
    ...
    role_box.fill.solid()
    role_box.fill.fore_color.rgb = WHITE
    role_box.line.color.rgb = accent
    _set_text(role_box.text_frame, role, 13, True, accent, center=True)
...
def _draw_evidence_motif(slide, left, top, width, height) -> None:
    for index in range(4):
        ...
        bar.fill.solid()
        bar.fill.fore_color.rgb = TEAL if index in {0, 2} else BLUE
        bar.line.color.rgb = bar.fill.fore_color.rgb
...
def _add_source_footer(slide, slide_spec: SlideSpec, *, evidence_by_id: dict[str, EvidenceItem]) -> None:
    footer = _compact_source_footer(slide_spec, evidence_by_id=evidence_by_id)
    if footer:
        _add_textbox(slide, Inches(0.8), Inches(6.95), Inches(11.6), Inches(0.25), footer, 8, False, GRAY)
